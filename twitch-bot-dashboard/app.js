(() => {
  ...
  function renderCustoms(){
    ...
    Object.keys(customCommands).forEach(name => {
      ...
      const del = document.createElement('button'); del.textContent='Del'; del.style.background='#ff6b6b'; del.addEventListener('click', ()=>{ delete customCommands[name]; persistCustoms(); renderCustoms(); });
      row.appendChild(label); row.appendChild(btn); row.appendChild(del); els.customCmds.appendChild(row);
    });
  }

  function persistCustoms(){ localStorage.setItem('customCommands', JSON.stringify(customCommands)); }

  function loadCustoms(){ try{ const s = localStorage.getItem('customCommands'); if(s){ const obj = JSON.parse(s); Object.assign(customCommands, obj); renderCustoms(); } }catch(e){console.warn(e);} }
  ...
  function connect(){
    const url = els.wsUrl.value || 'ws://localhost:8080';
  ...
  els.addCmdBtn.addEventListener('click', ()=>{
    const name = (els.newCmdName.value||'').trim(); const payload = (els.newCmdPayload.value||'').trim(); if(!name || !payload) return; customCommands[name]=payload; els.newCmdName.value=''; els.newCmdPayload.value=''; persistCustoms(); renderCustoms();
  });
  ...
  window.addEventListener('load', () => {
    els.wsUrl.value = localStorage.getItem('wsUrl') || 'ws://localhost:8080';
